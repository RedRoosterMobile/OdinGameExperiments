...
MIN_PLAYER_POSITION :: 5
MAX_PLAYER_POSITION :: 20
...
randIntn :: proc(min: i32, max: i32) -> int {
			...
			if !ball_on_air {
				...
				if update_ball(player_turn) {
					...
					if left_team_alive && right_team_alive {
						...
						ball.active = false
						player_turn += 1
						if player_turn == int(MAX_PLAYERS) {
							player_turn = 0
						}
					} else {
						game_over = true
		...
		for i in 0 ..< MAX_PLAYERS {
			if player[i].is_alive {
				...
				raylib.DrawRectangle(
					int(player[i].position.x - player[i].size.x / 2),
					int(player[i].position.y - player[i].size.y / 2),
					int(player[i].size.x),
					int(player[i].size.y),
					color,
				)
...
init_buildings :: proc() {
	...
	for i in 0 ..< MAX_BUILDINGS {
		...
		building[i].rectangle.width = f32(
			randIntn(
				int(building_width_mean * f32((100 - BUILDING_RELATIVE_ERROR / 2) / 100 + 1)),
				int(building_width_mean) * int((100 + BUILDING_RELATIVE_ERROR) / 100),
			),
		)

		current_width += int(building[i].rectangle.width)
		current_heighth: int = randIntn(BUILDING_MIN_RELATIVE_HEIGHT, BUILDING_MAX_RELATIVE_HEIGHT)
		building[i].rectangle.y = f32(screen_height - i32((int(screen_height) * int(current_heighth) / 100)))
		building[i].rectangle.height = f32(int(screen_height) * current_heighth / 100 + 1)
		gray_level: int = randIntn(BUILDING_MIN_GRAYSCALE_COLOR, BUILDING_MAX_GRAYSCALE_COLOR)
	...
	for i in 0 ..< MAX_PLAYERS {
		...
		if player[i].is_left_team {
			player[i].position.x = f32(
				i32(randIntn(
					i32(screen_width * i32(MIN_PLAYER_POSITION) / i32(100)),
					i32(screen_width * MAX_PLAYER_POSITION / 100),
				)),
			)
		} else {
			player[i].position.x = f32(
	...
	if raylib.GetMousePosition().y <= player[player_turn].position.y {
		if player[player_turn].is_left_team &&
		   raylib.GetMousePosition().x >= player[player_turn].position.x {
			...
			player[player_turn].aiming_angle = int(
				math.asin(
...
update_ball :: proc(player_turn: int) -> bool {
	explosion_number: int = 0

	if !ball.active {
		if player[player_turn].is_left_team {
			ball.speed.x = f32(
				math.cos(f64(player[player_turn].previous_angle) * raylib.DEG2RAD) *
				f64(player[player_turn].previous_power * 3 / int(DELTA_FPS)),
			)
			ball.speed.y = f32(
